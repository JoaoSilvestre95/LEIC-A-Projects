...
namespace xpl {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    std::vector<std::string> _externs; //Provides function calls
    ...
    std::shared_ptr<xpl::symbol> _current_func;
  ...
  private:
    inline std::string identifierFunction(std::string identifier){
        if(identifier == "xpl")
            return "_main";
        if(identifier == "_main")
            return "xpl";
    ...
    /*inline std::string identifierFunction(xpl::function_declaration_node * const node){
        return identifierFunction(node->identifier());
    }
    inline std::string identifierFunction(xpl::function_definition_node * const node){
        return identifierFunction(node->identifier());
    }
    inline std::string identifierFunction(xpl::function_invocation_node * const node){
        return identifierFunction(node->identifier());
    }*/
  ...
  public:
    ...
    void do_function_invocation_node(xpl::function_invocation_node * const node, int lvl);
